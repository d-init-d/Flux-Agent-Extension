...
const notificationStore: Map<string, chrome.notifications.NotificationOptions> = new Map();
...
function createStorageArea(store: Record<string, unknown>) {
  const onChanged =
    createMockEvent<(changes: Record<string, chrome.storage.StorageChange>) => void>();
...
function createMockTabs() {
  const onUpdated =
    ...
    >();
  const onRemoved =
    createMockEvent<(tabId: number, removeInfo: chrome.tabs.TabRemoveInfo) => void>();
  const onActivated = createMockEvent<(activeInfo: chrome.tabs.TabActiveInfo) => void>();
  ...
  return {
    ...
    captureVisibleTab: vi.fn(
      (windowId?: number, options?: chrome.tabs.CaptureVisibleTabOptions): Promise<string> => {
        const format = options?.format ?? 'png';
        return Promise.resolve(`data:image/${format};base64,mockScreenshotBase64Data`);
      },
    ),
...
function createMockRuntime() {
  const onMessage =
    ...
    >();
  const onInstalled = createMockEvent<(details: chrome.runtime.InstalledDetails) => void>();
  ...
  return {
    ...
    connect: vi.fn((connectInfo?: chrome.runtime.ConnectInfo): chrome.runtime.Port => {
...
function createMockDebugger() {
  const onEvent =
    createMockEvent<(source: chrome.debugger.Debuggee, method: string, params?: object) => void>();
  ...
  return {
    ...
    detach: vi.fn((target: chrome.debugger.Debuggee): Promise<void> => {
      const key = JSON.stringify(target);
...
function createMockCookies() {
  const onChanged = createMockEvent<(changeInfo: chrome.cookies.CookieChangeInfo) => void>();

  return {
    get: vi.fn((details: chrome.cookies.Details): Promise<chrome.cookies.Cookie | null> => {
    ...
    set: vi.fn((details: chrome.cookies.SetDetails): Promise<chrome.cookies.Cookie> => {
    ...
    remove: vi.fn((details: chrome.cookies.Details): Promise<chrome.cookies.Details | null> => {
      const index = cookieStore.findIndex((c) => c.name === details.name);
      if (index !== -1) {
...
function createMockWebNavigation() {
  return {
    onCompleted:
      createMockEvent<(details: chrome.webNavigation.WebNavigationFramedCallbackDetails) => void>(),
    onBeforeNavigate:
      createMockEvent<
...
function createMockNotifications() {
  ...
  const onClosed = createMockEvent<(notificationId: string, byUser: boolean) => void>();
...
function createMockAlarms() {
  ...
  return {
    create: vi.fn((name: string, alarmInfo: chrome.alarms.AlarmCreateInfo): Promise<void> => {
    ...
    get: vi.fn((name: string): Promise<chrome.alarms.Alarm | undefined> => {
